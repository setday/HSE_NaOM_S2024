...
namespace ADAAI::Exp::Core::Fourier
{
  ...
  /// \brief Computes exp(value) using the Fourier series approximation.
  ...
  template<typename T>
    requires std::is_floating_point_v<T>
  constexpr T Exp_Fourier( T value )
  {
    if ( !is_initialized )
    {
      FFT();
    ...
    size_t i          = 0;
    double prev_delta = std::abs( value - result[i] );
    for ( ; i <= N; ++i )
    {
      double delta = std::abs( value - result[i] );
