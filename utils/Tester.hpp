...
namespace ADAAI::Utils {
    ...
    CheckObject range_check(T left, T right, T step, bool break_on_fail = false) {
        static_assert(std::is_base_of_v<CheckObjectBase<T>, CheckObject>,
                      "CheckObject must be derived from CheckObjectBase");

        CheckObject result;

        result.test_case_number = ++TEST_NUMBER;

        result.test_data = "Range check in [" + std::to_string(left) + ", " + std::to_string(right) + "] with step " +
                           std::to_string(step);
